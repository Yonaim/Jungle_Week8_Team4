...
void FGPUProfiler::Shutdown()
{
    ...
    for (uint32 f = 0; f < FRAME_COUNT; ++f)
    {
        if (Frames[f].DisjointQuery)
        {
            Frames[f].DisjointQuery->Release();
            Frames[f].DisjointQuery = nullptr;
        }

        for (uint32 i = 0; i < MAX_TIMESTAMPS; ++i)
        {
            if (Frames[f].Timestamps[i].BeginQuery)
            {
                Frames[f].Timestamps[i].BeginQuery->Release();
            }
            if (Frames[f].Timestamps[i].EndQuery)
            {
                Frames[f].Timestamps[i].EndQuery->Release();
            }
...
{
    if (!bInitialized || !bFrameActive || Index == UINT32_MAX)
        return;

    FFrameData& Write = Frames[WriteIndex];
    if (Index >= Write.UsedCount)
        return;
